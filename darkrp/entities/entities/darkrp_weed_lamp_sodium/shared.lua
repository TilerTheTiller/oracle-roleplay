...
ENT.PrintName = "Sodium Grow Lamp"
...
function ENT:Initialize()
	if SERVER then
		self:SetModel("models/zerochain/props_growop2/zgo2_sodium_lamp01.mdl")
...
if CLIENT then
	...
	function ENT:Draw()
		...
		-- Dynamic light effect (warmer color for sodium)
		if self:GetIsOn() and self:GetHasPower() then
			if self.LightDynamic then
				self.LightDynamic.pos = self:GetPos() + self:GetUp() * 10
				self.LightDynamic.r = 255
				self.LightDynamic.g = 200
				self.LightDynamic.b = 100
				self.LightDynamic.brightness = 4
		...
		local statusColor = self:GetIsOn() and Color(255, 200, 100) or Color(200, 200, 200)
		if not self:GetHasPower() then
			statusColor = Color(255, 100, 100)
		end
		
		cam.Start3D2D(pos, Angle(0, ang.y, 90), 0.08)
			draw.SimpleText("Sodium Lamp", "DarkRP.Weed.Small", 0, -30, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
